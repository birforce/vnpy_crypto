...
class Strategy45(CmaTemplate):
    ...
    def __init__(self, cmaEngine, setting=None):
        ...
        self.paramList.append('inputSS')  # 下单范围
        self.paramList.append('min_trade_size')  # 下单最小成交单位
        self.varList.append('m_pos')  # 主交易所交易货币仓位
        self.varList.append('s_pos')  # 从交易所交易货币仓位
        ...
        self.curTick = None  # 当前Tick

        self.m_pos = EMPTY_STRING  # 主交易所交易货币仓位
        self.s_pos = EMPTY_STRING  # 从交易所交易货币仓位

        self.last_master_tick = None  # 主交易所比对得最后一个tick
        self.last_slave_tick = None  # 从交易所比对得最后一个tick
        self.master_base_pos = None  # 主交易所交易主货币持仓
        self.master_quote_pos = None  # 主交易所基准货币持仓
        self.slave_base_pos = None  # 从交易所交易主货币持仓
        self.slave_quote_pos = None  # 从交易所基准货币持仓
        ...
        if setting:
            ...
            lineDiffSetting['barTimeInterval'] = 60  # bar時間間隔
    ...
    def onInit(self, force=False):
        """初始化
        从sina上读取主交易合约和远期合约，合成价差
        """
        if force:
            self.writeCtaLog(u'策略强制初始化')
            self.inited = False  # 策略初始化状态
            self.trading = False  # 启动交易状态
        else:
        ...
        if not m_rt or not s_rt:
            self.writeCtaError(u'初始化数据失败,{}:{},{}:{}'.format(self.master_exchange, u'成功' if m_rt else u'失败',
    ...
    def buy(self, volume):
        ...
        # master_quote_pos：主市场基准货币持仓；slave_base_pos：从市场交易主货币持仓
        ...
        # 从市场交易货币持仓.仓位数量 - 从交易所比对的最后一个tick.冻结数量 < 卖出数量
    ...
    def sell(self, volume):
        ...
        # master_base_pos：主市场交易货币持仓；slave_quote_pos：从市场基准货币持仓
        ...
        if self.slave_quote_pos.longPosition / self.last_slave_tick.lastPrice < volume:
            self.writeCtaLog(
    ...
    def update_pos_info(self):
        ...
        # master_base_pos：主交易主货币持仓
        if self.master_base_pos:
            self.m_pos += u'[{}:{},Fz:{}]'.format(self.base_symbol, self.master_base_pos.longPosition,
                                                  self.master_base_pos.frozen)
        # master_quote_pos：主基准货币持仓
    ...
    def onTick(self, tick):
        ...
        if tick.vtSymbol == self.master_symbol and self.inited:
            ...
            # 主交易所交易货币持仓 = 持仓缓存字典获取交易货币合约代码和主交易所为空的持仓
            self.master_base_pos = self.cmaEngine.posBufferDict.get(
                '.'.join([self.base_symbol, self.master_exchange]),
                None)

            # 主交易所基准货币持仓 = 持仓缓存字典获取基准货币合约代码和主交易所为空的持仓
            self.master_quote_pos = self.cmaEngine.posBufferDict.get(
        ...
        # vt系统合约代码 == 从交易所币对；
            ...
            self.slave_base_pos = self.cmaEngine.posBufferDict.get(
                '.'.join([self.base_symbol, self.slave_exchange]),
                None)
            # 从交易所基准货币持仓 = 持仓缓存字典基准货币合约代码和从交易所为空的持仓
            self.slave_quote_pos = self.cmaEngine.posBufferDict.get(
        ...
        if not (self.inited and len(self.lineDiff.lineMiddleBand) > 0 and len(self.lineRatio.lineStateMean) > 0 and len(
