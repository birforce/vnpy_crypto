...
from vnpy.trader.app import cmaStrategy

# 初始化的接口模块，以及其指定的名称,CTP是模块，value，是该模块下的多个连接配置文件,如 CTP_JR_connect.json    'CTP_Prod', 'CTP_JR', , 'CTP_JK', 'CTP_02'
init_gateway_names = OrderedDict({'BINANCE':['BINANCE_2'],'FCOIN':['FCOIN_3']})
gateway_model_dict = {'BINANCE': binanceGateway,'FCOIN':fcoinGateway}


AUTO_CONNCET_GW = ['BINANCE_2','FCOIN_3']
if 'gateway_name' in globalSetting:
    AUTO_CONNCET_GW = globalSetting['gateway_name']
    print('use GW:{}'.format(AUTO_CONNCET_GW))
########################################################################
...
def runServer():
    ...
    try:
        log_file_name = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                                 'logs', u'runServer.log'))
    except Exception as ex:
        print( u'Use local dict:{0}'.format(os.getcwd()))
        log_file_name = os.path.abspath(os.path.join(os.getcwd(), 'logs', u'runServer.log'))
